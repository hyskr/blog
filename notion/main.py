...
def get_title(page):
    title = page["properties"].get("Name") or page["properties"].get('title') or page["properties"]['标题'] or ""
    # title = title["title"][0]["plain_text"]
    return title["title"][0]["plain_text"] if title else ""
...
async def update_file(path, block_id=None, page=None, forceUpdate=False):
    ...
    try:
        multi_tags = page["properties"].get("Tags") or page["properties"].get('tags') or page["properties"]['标签']
        multi_tags = multi_tags.get("multi_select")
        tags = [tag['name'] for tag in multi_tags]
        if front_matter.get('tags') != tags:
    ...
    try:
        description = page["properties"].get("Description") or page["properties"].get('description')  or page["properties"]['描述']
        description = description["rich_text"][0]["plain_text"] if description["rich_text"] else ""
        if front_matter.get('description') != description:
    ...
    try:
        with open(f'{block_id}_tmp/{block_id}.md', 'r', encoding='utf-8') as f:
            content = f.read()
...
async def update_list(file_path):
    ...
    if os.path.isfile(os.path.join(dir_path, "notion/.notion_files")):
        with open(os.path.join(dir_path, "notion/.notion_files"), "r", encoding='utf-8') as f:
    ...
    with open(os.path.join(dir_path, "notion/.notion_files"), "w", encoding='utf-8') as f:
